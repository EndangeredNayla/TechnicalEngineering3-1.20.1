...
public abstract class CmTileMachine extends CmTileEntity implements ISlotAcceptor
{
    ...
    public void whenPlaceToWorld()
    {
        for(Direction d : Direction.values()) {
            info.setOpenEnergy(d, initialFaceMode(ForgeCapabilities.ENERGY));
            info.setOpenItem(d, initialFaceMode(ForgeCapabilities.ITEM_HANDLER));
            info.setOpenFluid(d, initialFaceMode(ForgeCapabilities.FLUID_HANDLER));
        }
    ...
    public void doBaseData()
    {
        ...
        if(getTileAliveTime() % 20 == 0) {
            ...
            for(Tank tank : tanks) {
                ...
                int id = BuiltInRegistries.FLUID.getId(tank.getFluid().getFluid());
    ...
    {
        if(Objects.equals(cap, ForgeCapabilities.ITEM_HANDLER)) {
            if(side == null) {
        ...
        if(Objects.equals(cap, ForgeCapabilities.FLUID_HANDLER)) {
            if(side == null) {
