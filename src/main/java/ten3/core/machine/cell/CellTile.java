...
public class CellTile extends CmTileMachine
{
    ...
    {
        IEnergyStorage e = s.getCapability(ForgeCapabilities.ENERGY).orElse(null);
        if(e != null) {
    ...
    public void update()
    {
        ...
        if(stack0.getCount() == 1) {
            stack0.getCapability(ForgeCapabilities.ENERGY).ifPresent(
                    (e) -> {
                        if(e.canExtract()) {
        ...
        if(stack1.getCount() == 1) {
            stack1.getCapability(ForgeCapabilities.ENERGY).ifPresent(
                    (e) -> {
                        if(e.canReceive()) {
